...
foreach($chargeCodes as $_chargeCode)
{
	$id = $_chargeCode['ChargeCode']['id'];
	$stats['ChargeCode.'.$id] = [
		'name' => $_chargeCode['ChargeCode']['name'],
		'value' => 0,
		'color' => str_replace('#', '', $_chargeCode['ChargeCode']['color_code_hex']),
	];
}
...
$scope = __('All');
if(isset($chargeCode['ChargeCode']['id']))
{
	$scope = $chargeCode['ChargeCode']['name'];
}
